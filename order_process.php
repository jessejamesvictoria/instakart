...
define('CLIENT_ID', 'AYfDR36xha_XSAWr06-AELeVa_8y-dcsNQUHqlRMk4UZ_sr4x-Guw1c20AolcfloY-bhBvCYv_I6Kfy2'); //your PayPal client ID
define('CLIENT_SECRET', 'EJzqiXoHuTSgSIUDqD39d_L-Ou35Lg1GJ-DMOk79SfsUfB0ygpPcfaeWShPSe-oqgyrk8rZ_niw_6_Kb'); //PayPal Secret
define('RETURN_URL', 'http://instakart.rf.gd/order_process.php'); //return URL where PayPal redirects user
define('CANCEL_URL', 'http://instakart.rf.gd/payment_cancel.html'); //cancel URL
...
if(isset($_POST["item_code"])){
	...
	$item_name = addslashes(strip_tags($_POST["item_name"])); //get item code
	$item_name = mysqli_real_escape_string($conn, $item_name);
	$item_code = addslashes(strip_tags($_POST["item_code"])); //get item code
	$item_code = mysqli_real_escape_string($conn, $item_code);
	$item_price = addslashes(strip_tags($_POST["item_price"])); //get item price
		...
		if($result->state == "approved"){ //if state = approved continue..
			...
			$shipping_country_code	= $result->transactions[0]->item_list->shipping_address->country_code;
			  ...
			  /*
				...
				$insert_row = $mysqli->query("INSERT INTO transactions (payment_id, payer_name, payer_email, transaction_id)
					VALUES ('payer_first_name' , '$payer_email' , '$transaction_id')");
				*/
				if($transaction_amount != 50 && $transaction_amount  != 100 && $transaction_amount != 200 && $transaction_amount != 500 && $transaction_amount != 1000 && $transaction_amount != 2000)
				{
				...
				{
					$insert_pp = "INSERT INTO pp_payments (payer_name, payer_email, transaction_id, amount) VALUES ('$payer_first_name','$payer_email', '$transaction_id', $transaction_amount)";
					mysqli_query($conn, $insert_pp);
					$uid = $_SESSION['uid'];
					...
					$insert_balance_query = "UPDATE balance SET balance = $new_balance WHERE uid= $uid";
					mysqli_query($conn, $insert_balance_query);
				}	
